...
def get_example_prompt():
    prompt = "transparent umbrella, RAW photo, subject, 8k uhd, dslr, high quality, Fujifilm XT3, half-length portrait from knees up, scarlett, short red hair, blue eyes, school uniform, white shirt, red tie, blue pleated microskirt"
...
def base_lora(lora_path: str, lora_name: str, bath_fix: str = "1"):
    ...
    if lora_name != '':
        pipeline.load_lora_weights(lora_path, weight_name=lora_name, adapter_name="base_lora_style")
        cur_loras = ["base_lora_style"]
        pipeline.set_adapters(cur_loras)
