...
def generate_image(lora_type: str, lora_list: list, method: str, speed_type: str, prompt, negative_prompt):
    ...
    if method == "merge":
        pipeline.set_adapters(cur_loras, lora_weights)
        switch_callback = None
    elif method == "switch":
        pipeline.set_adapters([cur_loras[0]], [lora_weights[0]])
    ...
    image.save(
        f"/kaggle/working/Multi-LoRA-Composition/test_file_image/{lora_type}-{method}-{name_1}-{method}-{speed_type}-loadLora3.jpg")
    pipeline.unload_lora_weights()
    pipeline.disable_lora()
    return (f"/kaggle/working/Multi-LoRA-Composition/test_file_image/{lora_type}-{method}-{name_1}-{method}-{speed_type}-loadLora3.jpg",
